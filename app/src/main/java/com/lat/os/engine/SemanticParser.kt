...
object SemanticParser {
    private val patterns = listOf(
        Regex("open (.+)", RegexOption.IGNORE_CASE) to { match: MatchResult ->
            Action("OPEN_APP", match.groupValues[1])
        },
        ...
        Regex("click (.+)", RegexOption.IGNORE_CASE) to { match: MatchResult ->
            Action("TAP_TEXT", match.groupValues[1])
        },
        Regex("type (.+)", RegexOption.IGNORE_CASE) to { match: MatchResult ->
            Action("TYPE_TEXT", match.groupValues[1])
        }
    )
